...
def FeydySink(born_samples_all, data_samples_all, epsilon):
    ...
    sinkhorn_feydy = feydy_sink.sinkhorn_divergence(
        born_probs_tens, born_samples_tens, data_probs_tens, data_samples_tens, eps=epsilon)
...
def SinkGrad(born_samples, born_samples_pm, data_samples, epsilon):
    ...
    g_data, f_born = feydy_sink.sink(born_probs_tens, born_samples_tens, data_probs_tens, data_samples_tens,
                                     eps=epsilon)  # Compute optimal dual vectors between born samples and data
    _, s_born = feydy_sink.sym_sink(born_probs_tens, born_samples_tens,
                                    eps=epsilon)  # Compute autocorrelation vectors for Born data

    p = 2
    # Sinkhorn Cost will be Euclidean Distance squared, or l_2 norm squared.
    # Equivalent to Hamming Distance in this case.

    cost_matrix_plus_born = feydy_sink.dist_matrix(born_plus_samples_tens, born_samples_tens, p,
                                                   epsilon)  # Compute cost matrix between born_plus and born samples
    cost_matrix_minus_born = feydy_sink.dist_matrix(born_minus_samples_tens, born_samples_tens, p,
                                                    epsilon)  # Compute cost matrix between born_minus and born samples

    cost_matrix_plus_data = feydy_sink.dist_matrix(born_plus_samples_tens, data_samples_tens, p,
                                                   epsilon)  # Compute cost matrix between born_plus and data samples
    cost_matrix_minus_data = feydy_sink.dist_matrix(born_minus_samples_tens, data_samples_tens, p,
                                                    epsilon)  # Compute cost matrix between born_minus and data samples
    ...
    f_minus = -epsilon * feydy_sink.lse(
        (g_data + data_probs_tens.log().view(1, -1)).view(1, -1) - cost_matrix_minus_data)
    s_minus = -epsilon * feydy_sink.lse(
        (s_born + born_probs_tens.log().view(1, -1)).view(1, -1) - cost_matrix_minus_born)

    feydy_sink_grad = feydy_sink.scal(born_minus_probs_tens, f_minus - s_minus) - feydy_sink.scal(born_plus_probs_tens,
                                                                                                  f_plus - s_plus)
...
def _gamma_(f, p, g, q, M, sinkhorn_eps=.01):
    ...
    """
    return ((f.view(-1, 1) + g.view(1, -1)) / sinkhorn_eps - M / sinkhorn_eps +
            p.log().view(-1, 1) + q.log().view(1, -1)).exp()
